...
The labeled lines are prompt scaffolding, not a closed schema. `Asset type` and `Input images` are prompt-only scaffolding; the helper does not expose them as dedicated flags.

Execution details such as helper flags, `quality`, `input_fidelity`, masks, output formats, env files, deployment names, API versions, and local output paths depend on mode. Use the Azure helper by default, including simple transparent-image requests. For transparent images, prompt for a flat chroma-key background and remove it locally with `python "${CODEX_HOME:-$HOME/.codex}/skills/.system/imagegen/scripts/remove_chroma_key.py"`.

Azure model notes:
- Use the deployment configured by `AZURE_OPENAI_DEPLOYMENT`; do not assume it has the same name as the public model.
- For GPT Image 2 deployments, prefer concrete dimensions such as `1024x1024`, `1536x1024`, `1024x1536`, `2048x2048`, `2048x1152`, `3840x2160`, or `2160x3840` when supported by the Azure deployment/API version.
- Use `quality=low` for drafts and thumbnails; use `medium`, `high`, or deployment-supported defaults for final assets.
- If true native transparency is required, ask before changing Azure deployment/model assumptions. The portable default is chroma-key generation plus local removal.
...
Post-process note: after Azure generation, run `python "${CODEX_HOME:-$HOME/.codex}/skills/.system/imagegen/scripts/remove_chroma_key.py" --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill`. Ask before changing Azure deployment/model assumptions for true/native transparency, failed chroma-key validation, or complex subjects such as hair, fur, glass, smoke, liquids, translucent materials, reflections, or soft shadows.
