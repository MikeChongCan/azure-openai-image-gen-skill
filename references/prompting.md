...
- Azure helper mode (default)
- implementation mode for repo code that calls Azure OpenAI

This file is about prompt structure, specificity, and iteration. Azure helper execution controls such as `quality`, `input_fidelity`, masks, output format, deployment, env file, and output paths belong in helper flags or implementation code, not inside the image prompt.
...
- [Azure helper execution controls](#azure-helper-execution-controls)
...
- In Azure helper mode, use `medium` or `high` quality for small text, dense infographics, data-heavy slides, multi-font layouts, legends, axes, and footnotes.
...
## Transparent images
- Use Azure helper mode first for transparent-image requests. If the subject is clearly too complex for chroma-key removal, explain that true model-native transparency depends on the deployed Azure model/API version and ask before changing deployment/model assumptions.
...
- Do not silently switch Azure deployments for true/native transparency. Ask first for true/native transparency requests, failed chroma-key validation, or complex transparent subjects such as hair, fur, glass, smoke, liquids, translucent materials, reflective objects, or soft shadows.

## Azure helper execution controls
- `quality`, `input_fidelity`, explicit masks, output format, env file, deployment, API version, and output paths are Azure helper execution controls.
- Do not put model routing or Azure credentials in the prompt.
- See `references/azure-openai-image-api.md` for endpoint, parameter, and model-availability notes.
...
- Use `quality=low` for fast drafts and thumbnails; use `medium`, `high`, or deployment-supported defaults for final assets.
- If the user asks for 4K-style output with a deployment that supports GPT Image 2 dimensions, use `3840x2160` for landscape or `2160x3840` for portrait.
...
- infographic-diagram: Define the audience and layout flow; label parts explicitly; require verbatim text; prefer higher quality in Azure helper mode for dense labels.
...
- background-extraction: For simple opaque subjects, request a clean cutout on a perfectly flat chroma-key background; crisp silhouette; generous padding; no shadows; no halos; preserve label text exactly; no restyling. Ask before changing Azure deployment/model assumptions for true native transparency.
